...
(
  cd build/package
  ditto -c -k --sequesterRsrc --keepParent Payload ../../dist/AndroidPessoal-unsigned.ipa
)
shasum -a 256 dist/AndroidPessoal-unsigned.ipa > dist/AndroidPessoal-unsigned.ipa.sha256

bundle_id=$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$app/Info.plist")
minimum_ios=$(/usr/libexec/PlistBuddy -c 'Print :MinimumOSVersion' "$app/Info.plist")
version=$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$app/Info.plist")
sha=$(awk '{print $1}' dist/AndroidPessoal-unsigned.ipa.sha256)
size=$(stat -f '%z' dist/AndroidPessoal-unsigned.ipa)
...
{
  "artifact": "AndroidPessoal-unsigned.ipa",
  "signed": false,
  "bundleId": "$bundle_id",
  "version": "$version",
...
JSON
unzip -l dist/AndroidPessoal-unsigned.ipa
